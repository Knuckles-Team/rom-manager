#!/usr/bin/env python3
"""API-to-MCP integration parity check.

rom-manager is a **local** tool (archive extraction + chdman/dolphin-tool
conversion), not a REST API. This adaptation therefore does NOT require any REST
endpoints. It verifies that the public methods of the local ``Api`` facade in
``api_client.py`` are genuinely wired into the MCP tools — even when those tools
live in dedicated tool modules (e.g. ``rom_manager/mcp/*.py``) rather than in
``mcp_server.py`` itself — so the pass is real, not a no-op.
"""
...
def verify_agent(agent_dir):
    ...
    # Tools may live directly in mcp_server.py and/or in dedicated tool modules
    # (e.g. an ``mcp/`` package, or files named ``mcp_*.py``). Scan all of them
    # and aggregate the coverage so the parity check reflects the real wiring.
    tool_module_paths = set(mcp_servers)
    for pattern in ("mcp_*.py", os.path.join("mcp", "*.py")):
        tool_module_paths.update(
            glob.glob(os.path.join(agent_dir, "**", pattern), recursive=True)
        )

    tool_mappings = {}
    mapped_methods = set()
    for module_path in sorted(tool_module_paths):
        if not os.path.isfile(module_path):
            continue
        module_maps, module_mapped = parse_mcp_server(module_path, api_methods)
        tool_mappings.update(module_maps)
        mapped_methods.update(module_mapped)

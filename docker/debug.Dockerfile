...
# Install base dependencies, conversion CLIs, uv, and starship shell prompt
RUN apt-get update \
    && apt-get install -y --no-install-recommends ripgrep tree fd-find curl nano build-essential cmake libssl-dev libcurl4-openssl-dev pkg-config mame-tools p7zip-full \
    && apt-get clean \
